...
contract Controller {
  ...
  event BeaconCreated(string indexed name, BeaconType beaconType, address addr);
  ...
  function createProxy(string memory name) public returns (address) {
    ...
    assembly {
      ...
      let masked := and(code, 0x0000000000000000000000000000000000000000ffffffffffffffffffffffff)
  ...
  function _deployBeacon(string memory name, BeaconType beaconType) internal returns(address) {
    require(msg.sender == admin);
  ...
  function _destroyBeacon(string memory name, BeaconType beaconType) internal {
    address beaconAddress = _getBeaconAddress(name, beaconType);
    Beacon(beaconAddress).destroy();
  }

  function _getBeaconCode(string memory name) internal view returns (bytes memory) {
    return abi.encodePacked(type(Beacon).creationCode, abi.encode(address(this), name));
  }

  function _getBeaconSalt(string memory name, BeaconType beaconType) internal pure returns (bytes32) {
    return keccak256(abi.encodePacked(beaconType, name));
  }

  function _getBeaconAddress(string memory name, BeaconType beaconType) internal view returns (address) {
    bytes32 bytecodeHash = keccak256(_getBeaconCode(name));
    bytes32 salt = _getBeaconSalt(name, beaconType);
    bytes32 dataHash = keccak256(abi.encodePacked(bytes1(0xff), address(this), salt, bytecodeHash));
    return address(uint256(dataHash));
  }
} 
